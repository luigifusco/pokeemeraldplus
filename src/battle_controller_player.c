...
static void Intro_WaitForShinyAnimAndHealthbox(void)
{
    ...
    if (healthboxAnimDone
     ...
     && (!hasPartner || gBattleSpritesDataPtr->healthBoxesData[BATTLE_PARTNER(gActiveBattler)].finishedShinyMonAnim))
    {
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    u32 expToNextLvl;
    ...
    #ifdef NEGATIVE_EXP
        // For EXP loss, animate exp bar decreasing. If we're at the start of a level and
        // still need to remove EXP, immediately drop a level so the bar starts full.
        while (gainedExp > 0 && exp == 0 && level > 1)
        {
                ...
                // Recompute for the new level, using the same total EXP.
                (void)oldLevel;
        ...
        // Only animate the part we can remove within the current level.
        if (gainedExp > exp)
                gainedExp = exp;
    ...
    else
    {
        ...
        if (newExpPoints == -1) // The bar has been filled with given exp points.
        {
...
#ifdef NEGATIVE_EXP
            {
                ...
                // The bar math uses receivedValue; for NEGATIVE_EXP this is the segment removed.
