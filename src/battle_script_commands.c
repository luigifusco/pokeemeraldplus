...
// Highest level a party Pokemon may gain EXP toward. GetCurrentLevelCap()
// (battle_setup.c) returns MAX_LEVEL unless LEVEL_CAP is enabled, so the EXP
// gate below behaves exactly like the vanilla "== MAX_LEVEL" check by default.
...
#ifdef NO_EXP
                    // Build-time toggle: prevent EXP gain.
...
static void Cmd_handleballthrow(void)
{
    ...
    // In double battles, if the target is fainted, try the partner
    if (gBattleMons[gBattlerTarget].hp == 0)
    {
        u8 otherTarget = BATTLE_PARTNER(gBattlerTarget);
        if (!(gAbsentBattlerFlags & gBitTable[otherTarget])
            && gBattleMons[otherTarget].hp != 0)
        {
            gBattlerTarget = otherTarget;
        }
    }
