...
_TRAINER_GRAPH_JS = (
    ...
    "}\n"
    "var pkTrLast='';var pkTrInv=false;\n"
    "function pkTrGroup(g){\n"
    "  if(g==='all'||g==='none'){pkTrLast='';pkTrInv=false;}\n"
    "  else if(g===pkTrLast&&!pkTrInv){pkTrInv=true;}\n"
    "  else{pkTrLast=g;pkTrInv=false;}\n"
    "  document.querySelectorAll('#pkTrList input').forEach(function(c){\n"
    "    if(g==='all'){c.checked=true;}else if(g==='none'){c.checked=false;}\n"
    "    else{var m=(TR_GRAPH[+c.value].g===g);c.checked=pkTrInv?!m:m;}\n"
    "  });\n"
    "  pkTrApply();\n"
    "}\n"
...
def write_community_board(communities, names, trainer_count, individual_count, colors, trainers) -> None:
    ...
    tjs = (
        ...
        "  box.innerHTML=h;\n"
        "}\n"
        "var trLast='';var trInv=false;\n"
        "function trGroup(g){\n"
        "  if(g==='all'||g==='none'){trLast='';trInv=false;}\n"
        "  else if(g===trLast&&!trInv){trInv=true;}\n"
        "  else{trLast=g;trInv=false;}\n"
        "  document.querySelectorAll('#trList input').forEach(function(c){\n"
        "    if(g==='all'){c.checked=true;}else if(g==='none'){c.checked=false;}\n"
        "    else{var m=(TR[+c.value].g===g);c.checked=trInv?!m:m;}\n"
        "  });\n"
        "  trRender();\n"
        "}\n"
